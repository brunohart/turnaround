...
Execute by invoking the `turnaround-build` skill (`~/.claude/skills/turnaround-build/SKILL.md`) with no argument; it derives the day from the date. It works in `/Users/brunohart/Developer/turnaround`, builds and tests with uv, regenerates and screenshots the week sheets, commits, pushes to `origin main`, and closes the day's Linear issue in project "Turnaround — the showtime grid solver".
...
Note: a launchd agent (`com.designedbybruno.turnaround-slot`, 08:45 Pacific/Auckland) also fires `~/Developer/turnaround/scripts/slot.sh`, which is idempotent per calendar day via `build/slots/<date>.ran`. If you enable this scheduled task in the app as well, unload the launchd agent to avoid two runs:
