...
## ADR-012 — The turnaround is its own interval, and the booth's rules are policy, not terms

**Decided:** 2026-09-10 (Day 4).
**Decision:** A session's turnaround begins `credits_min` before the feature ends and lasts the screen's clean time; the room is clear at the later of the feature's end and the turnaround's end, so the next preshow never starts over the last reel even when the credits outlast the clean. `Session.clear` carries that figure and the block a session occupies is `preshow + runtime + clean − credits`, floored at `preshow + runtime` (ADR-005 still holds: the turnaround lives inside the block). The preshow is the house's or the format's (`preshow_by_format`). The floor's capacity (`max_concurrent_turnarounds`), a screen's own hours (`screen.open`, `screen.last_start`) and the stagger window (`stagger_min` wide, `max_starts_per_window` in it, windows anchored at starts and half-open) are **house policy**: hard, never relaxed, never in a conflict, and not in `RELAXABLE`. When they make a day infeasible the solver blames the distributor terms that no longer fit, and the explanation states the house, staff included.
**Why:** The turnaround as an interval of its own is what lets the staff cap be one `add_cumulative` and the checker one count, with credits overlap falling out of the same arithmetic instead of a special case. Policy is what the house can change by rostering another usher or opening a room earlier; a term is what a distributor has to agree to. Relaxing policy silently would be ADR-002's sin by another door; relaxing it loudly is a different tool (a what-if, Day 7), not a `--relax`.
**Cost:** A day that fails only for want of an usher names a distributor's term as the culprit, which is true and unhelpful; Day 7's what-if is where "with three ushers this fits" belongs. A window that allows two starts also allows two at the same minute; a second, tighter window is a later field. The cumulative is omitted when the cap is not below the screen count, so a house of two screens with a cap of two states a rule the model never carries; the checker still reports it.

## ADR-013 — A week term reaches a day as a debt; the exclusive lifts on a named day; the PLF lock is a day term

**Decided:** 2026-09-11 (Day 5).
**Decision:** Distributor terms stay in one document, `Terms`, and each carries a scope (`TERM_SCOPE`): a *day* term (`plf_lock` joins `min_shows`, `prime_shows`, `exclusive_screen`, the windows) is checked and relaxed per day as before; a *week* term (`min_shows_per_week`, `prime_shows_per_week`, `exclusive_until`) is checked at week scope and refused, in a sentence, by a brief that is one day. The week still unfolds into plain days (ADR-010). `exclusive_until` unfolds by setting `exclusive_screen` on every day through the named day; a day override still wins. A week minimum reaches a day as what it is *owed*: `solve_week` passes each day the term less what the days before delivered less the most the days after could (an upper bound from the day's own hours and blocks), and the day enforces that floor under an assumption literal named `min_shows_per_week 28 · 6 owed today`, so a conflict says what the week wanted of the day and `--relax` drops it for that day only. A week term that asks nothing of today is not in the model at all. The checker re-counts the week terms from the sessions alone and accepts a shortfall only where some day's grid declares the term given up. `plf_lock` means no other title on any room whose formats include PLF; a dark PLF room is allowed; it is refused at validation where the house has no PLF room. Validation errors are translated into sentences (`validation_sentences`) that name the title, the field, and what the booking can carry.
**Why:** Keeping the terms in one document is the trade's reality: the booking is one letter. Solving the week in one model would carry the week terms natively but would multiply the model by seven and rewrite every conflict and relaxation path written on Days 1–4; the debt keeps every day a plain `solve` with the checker's independence intact. The floor is the weakest condition under which the week can still be met, so a day is never asked for more than it must give.
**Cost:** The floor is necessary, not sufficient: the bound on later days is loose (hours and blocks, not the other titles' terms or the stagger), so the tail of the week can be asked to carry a debt it cannot, and the week fails on Wednesday where an even spread from Thursday would have fit. The objective, not the term, decides how the shows are spread; a distributor who wanted them spread has to say so per day. `exclusive_until` is inclusive of the named day, which is one reading of "until". A week term that is relaxed is relaxed on one day's grid and reported at week scope, which is right but takes two tables to read.

## ADR-014 — One HTML is the screen, the print and the phone; the booth strip is the second layout, not a second template

**Decided:** 2026-09-12 (Day 6, run on the 13th).
**Decision:** The sheet has one template per document (`sheet`, `week`, `terms`) and no build step, no PDF library and no JavaScript. Print is CSS: `@page pinup` (A3 landscape) carries the sheet, `@page strip` (A4 portrait) carries one page per screen, `@page letter` (A4 portrait) carries the terms, and Chrome breaks pages on the name change. The **booth strip** — one screen, its sessions in mono, the three-strip block turned vertical with doors, feature, turnaround and clear, and the dark minutes between — is rendered into every sheet by the `strips` macro from the same `day_context` rows the grid uses; it is hidden on a desk, shown on a phone in place of the grid, and printed after the pin-up. The phone rule is scoped to `screen` so print layout can never pick it up. Backgrounds print (`print-color-adjust:exact`) because the hatches are the information. Screenshots for the log come from `scripts/shot.py` over the DevTools protocol, never from `--window-size` alone below about 500px.
**Why:** The sheet is paper first (DESIGN.md). A second template for print or a PDF pipeline would drift from the screen sheet within a week and double every future day's rendering work (Day 7's why, Day 11's diff, Day 12's board). One markup with three layouts keeps the proof table, the terms and the strips identical wherever they are read, and the checker's verdicts are the same pixels on every one.
**Cost:** The printed PDF is heavy while the hatches are CSS gradients (Skia rasterises them; 33 MB for four pages). The week prints one A4 per screen per day, which suits a multiplex and is a lot of paper for a three-screen house; a per-screen week strip is a later layout. The phone's wide tables scroll sideways. Rotated panels blur table text a fraction on screen. `shot.py` is macOS Chrome only.


## ADR-015 — A session's *why* is the checker's; whether it is *forced* is the solver's claim, said so, and a show is a title in a room in a daypart

**Decided:** 2026-09-13 (Day 7).
**Decision:** Every session carries two kinds of explanation. The **why** — the terms the session helps satisfy (`min_shows 4/3`, `prime_shows 1/1`, `exclusive_screen 1`, its window), its daypart and rank, what it sells and what it turns away — is re-derived by the checker (`check.whys`) from the brief and the grid alone, with the same arithmetic as `admissions`, so a title's whys sum to its admissions row (ADR-003 holds: no solver code). Whether the session is **forced** — present in every grid — cannot be known without solving, so it is a probe: forbid the *show* and solve the day again, honouring whatever the grid already declares relaxed. `INFEASIBLE` means the terms force it and the conflict names which (`by: terms`); a proven-optimal grid that sells fewer seats means the objective forces it by that delta and the probe says what the room did instead (`by: objective`); an equally good grid means it is free (`by: free`); a probe that runs out of time says so (`by: unknown`) and a worse grid found but not proven is `unknown` too, its delta an upper bound. The result lives on the session as `forced`, round-trips through the grid JSON, and is labelled on the sheet and in the CLI as *the solver's claim, not the checker's*. The checker ignores it. A **show**, for the probe, is this title in this room in this daypart (`show_of`): on a five-minute grid forbidding one start would only nudge the session, and the programmer's question is about the show. `what-if` re-solves the day without some titles or under a changed policy and never relaxes a term (ADR-002); it prints the two days side by side and where the freed slots went. `plan --why` probes every session (one solve each) and `run.sh` does that for the hero example only.
**Why:** The sheet is pinned in a booth to be argued with. "Why is that there" has two honest answers — the terms it serves, which anyone can count, and whether the grid needs it, which only a solver can say — and the two must not be confused, or the checker's independence is spent on a claim it cannot check. Forbidding the show rather than the minute is what makes the answer mean something; forbidding 17:30 alone gives back 17:35 and "free".
**Cost:** A full probe is one solve per session (the Regent: fifteen, about a minute); a week is not probed at all, and `explain` takes one day of it. `unknown` is a real outcome under a time limit and appears in the counts. The `instead` clause is a heuristic reading of the grid without the show (same title moved, same room taken, slot dark) and can misname a reshuffle that touched three rooms. A show forced by the objective on one optimal grid may be free on another equally optimal one; the probe answers for the grid it was given.
