...
**Why marks (Day 7).** A session the terms force carries a navy corner at the bottom-right of its feature block; one the objective forces carries the same corner in ink at less than half strength. Nothing on the grid is orange but the turnaround. The by-title table prints each start with what it sells and, when probed, `FORCED` in a navy box or the admissions the day gives up without it; the proof table gains a *Forced* column naming the sessions each term forces, and a mono line under it says how many were probed and that the count is the solver's claim, not the checker's. The hover title on every block carries the session's why in one line.

**The festival sheet (Day 10).** A festival is the week sheet with venues for screens and ten pages instead of seven. The programme table runs the days across and the titles down: each screening as a time and a venue in mono, the guest's screening under a navy rule, the guest's days washed the prime navy, the days the print is away dark. A strand is a mono tag beside the title — a thin ink border, uppercase, tracked — and never a colour: six strands as six colours would be the paint chart the sheet refuses. On a day page the venues are the rows, the clashes the day paid for are named in a mono line under the grid, and the by-title and proof tables show the titles in town that day with the rest folded into one line, because a festival day is what plays, not the slate. The tag rides the booth strip and the hover title too; it does not ride the block, where a 15-hour ruler leaves no room for it. Nothing on the grid is orange but the turnaround, still.

**The re-plan sheet (Day 11).** A diff renders as the new grid's sheet with the old one showing through. A session that moved leaves a *ghost* where it was: a dashed ink outline on the fine ink hatch, nothing filled, its old start in mono, drawn under the live blocks on the row it left (a room move ghosts the old room). A session that was added carries a small orange stamp — `ADDED`, mono, rotated −4° — at the corner of its feature. That stamp is the one orange on the grid besides the turnaround, and it appears only on a re-plan sheet, because a re-plan is the turnaround of the sheet itself: on every other sheet the rule of the last paragraph holds unchanged. A session that was removed is not drawn; it is struck through in rust at the end of its title's row in the by-title table, and a moved session's old start is struck in grey before its new one. A removed title with no session left today keeps its row, even on a festival day whose fold would hide it. Under the grid, a *Re-plan* stamp heads a table of every change — what, title, was, now — beside the grids' claims (sessions, seats, objective, what was paid, what was given up) in mono. The header says what the sheet is against. The booth strip carries the stamp and the old time too; the ghost does not ride it.
